...
const Heading = forwardRefWithAs<HeadingProps, 'div'>(function Heading(
  ...
  let label = 'Link para este heading'
  if (typeof children === 'string') {
    label = 'Link para ' + children
  }
...
export const H2 = ({className, ...props}: HeadingProps) => (
  <Heading
    as="h2"
    className={cn(
      'my-6 font-display text-3xl font-bold leading-10 text-primary',
      className
    )}
    {...props}
  />
...
export const H3 = ({className, ...props}: HeadingProps) => (
  <Heading
    ...
    className={cn(
      className,
      'my-6 font-display text-2xl font-bold leading-9 text-primary'
    )}
    {...props}
  />
)
